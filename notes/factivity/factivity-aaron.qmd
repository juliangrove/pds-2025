...
The inference in (@exm-mo-left-inference) "projects" through negation, questions, and conditionals—environments that typically cancel entailments. []{@exm-think-positive} []{@exm-think-negative} []{@exm-think-question} Compare this with non-factive "think":
...
This projection behavior has motivated analyses where factive predicates encode this presupposition directly [@heim_presupposition_1983]. The presupposition must be satisfied for the sentence to have a truth value—creating what @strawson_referring_1950 called "presupposition failure" when violated.
...
From [`Grammar.Lexica.SynSem.Factivity`](https://juliangrove.github.io/pds/Grammar-Lexica-SynSem-Factivity.html), here's the entry for "knows":
...
4. **Semantic components**:
   - `epi i @@ x @@ p`: x's epistemic state regarding p at index i
...
-- Step 4: The purePP injects this into the State.Probability parameterized monad
...
Before compilation to Stan, PDS applies delta rules to simplify the computation. From [`Lambda.Delta`](https://juliangrove.github.io/pds/Lambda-Delta.html), the key rules for factivity involve state updates and control flow:
...
The prior determines how often "know" is used factively. From [`factivityPrior`](https://juliangrove.github.io/pds/src/Grammar.Lexica.SynSem.Factivity.html#factivityPrior):
...
After delta reduction (via the [`disjunctions`](https://juliangrove.github.io/pds/src/Lambda.Delta.html#disjunctions) rule), this becomes:
