...
1. **Lexical ambiguity**: Predicates have both factive and non-factive senses [@patel-grosz_dislocated_2014]
...
3. **The crucial branching**: `ITE (TauKnow s) ...` is an if-then-else that checks the factivity parameter:
   - `TauKnow s` extracts whether this token of "know" is factive from the discourse state
...
Before compilation to Stan, PDS applies delta rules to simplify the computation. From `Lambda.Delta`, the key rules for factivity involve state updates:
...
After delta reduction, this becomes:
...
- `verb_intercept_z`: The `TauKnow` values for each verb
...
2. **State dependencies become array indexing**: PDS's functional `TauKnow :: State -> Bool` becomes Stan's `verb_prob[verb[n]]`
...
2. **Modified prior**: Place TauKnow in the common ground update:
...
3. **Modified delta rules**: Move TauKnow handling to indices:
...
This suggests a cognitive architecture where semantic and world knowledge remain distinct [@lewis_pragmatic_2014].
