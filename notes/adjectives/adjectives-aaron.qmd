...
Every Stan program is organized into blocks that appear in a specific order:

1. **data block**: Declares the observed data that will be provided from outside
2. **parameters block**: Declares the unknown quantities we want to learn about
3. **transformed parameters block**: Computes derived quantities from the parameters
4. **model block**: Specifies priors and the likelihood of the data
5. **generated quantities block**: Computes quantities of interest after inference

Let's see what each block does with a simple example before diving into our adjective models:

```stan
// A minimal Stan program
data {
...
This simple program infers the mean and standard deviation of normally-distributed data. The data block declares what we observe, the parameters block declares what we want to learn, and the model block connects them through a likelihood function.

## Mapping lambda-terms to Stan code

Now that we understand Stan's structure, we can address the heart of our enterprise: translating semantic theories expressed as typed λ-terms into Stan programs that can be fit to data. This translation is non-trivial because λ-calculus and Stan operate at different levels of abstraction and with different computational paradigms.
...
With this foundation, we can now understand how to translate semantic theories into Stan code.

### Model 2: Vagueness and imprecision

Now that we understand Stan's structure, let's implement a more sophisticated model that captures the theoretical distinction between vagueness (uncertainty about standards) and imprecision (tolerance around thresholds).
