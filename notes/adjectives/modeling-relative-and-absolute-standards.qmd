...
The theoretical distinction between relative and absolute adjectives suggests different semantic structures. However, the current PDS implementation in Grammar.Lexica.SynSem.Adjectives does not include separate entries for relative vs. absolute adjectives.

The actual "tall" entry from PDS is:

```haskell
-- From Grammar.Lexica.SynSem.Adjectives (lines 45-48)
"tall" -> [ SynSem {
    syn = AP,
...
Note that this does not include context-dependent thresholds. The mixture model presented here represents a theoretical extension showing how such distinctions could be implemented.

### Theoretical extensions for relative vs absolute interpretations

While not implemented in the current PDS codebase, the theoretical distinction between relative and absolute adjectives would require different delta rules. The actual delta rules in Lambda.Delta handle state updates as follows:

```haskell
-- From Lambda.Delta (lines 153-169)
states :: DeltaRule
states = \case
...
This shows how thresholds like `DTall` are extracted from the discourse state, but there's no built-in mechanism for context-dependent thresholds. The mixture model below represents a hypothetical extension showing how relative/absolute distinctions could lead to different statistical dependencies.
