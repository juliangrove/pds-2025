...
Our first model addresses a fundamental question: how do we infer the degrees that gradable adjectives operate on? The norming study provides a clean test case where participants directly report degrees on scales.
...
parses1 = interpretations @Adjectives expr1 0
parses2 = interpretations @Adjectives expr2 0
-- Extract semantic terms from parses
s1' = sem (head (getList parses1))
q1' = sem (head (getList parses2))
...
This term undergoes several delta reductions. First, the `indices` rule extracts the height value from the discourse state:

```haskell
-- | Computes functions on indices (from Lambda.Delta).
indices :: DeltaRule
indices = \case
  Height (UpdHeight p _) -> Just p
  Height (UpdSocPla _ i) -> Just (Height i)
  -- Additional cases handle other index updates
  _                      -> Nothing
```

Applied to our term, this yields:

$$
\ct{Return}(\iota(\lambda d[h = d]))
$$

where $h$ represents Jo's actual height at index $i$. The $\iota$ operator then extracts this unique value, giving us:
...
**Input PDS:**
```haskell
-- Example PDS processing pipeline
-- Using actual functions from PDS:
-- interpretations :: Expr -> Int -> ParseAs m  (from Grammar.Parser)
-- ty :: Sig -> Term -> Typed  (from Lambda.Types)
-- asTyped :: Sig -> (Term -> Term) -> Typed -> Typed  (from Lambda.Types)
-- assert, ask :: Term -> Term  (from Lambda.Convenience)
-- (>>>) :: Term -> Term -> Term  (from Lambda.Convenience)
discourse' = ty tau $ assert s1' >>> ask q1'
scaleNormingExample = asTyped tau (betaDeltaNormal deltaRules . adjectivesRespond scaleNormingPrior) discourse'
```
...
1. Parse "how tall jo is" → $\ct{Return}(\iota(\lambda d[\ct{height}(i)(j) = d]))$
2. Apply `indices` rule → $\ct{Return}(\iota(\lambda d[h = d]))$ 
3. Apply iota extraction → $\ct{Return}(h)$
