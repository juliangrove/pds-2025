...
PDS applies delta-rules to simplify these complex λ-terms. As discussed in [the implementation section](../pds-intro/delta-rules.qmd), delta-rules enable different semantic computations while preserving semantic equivalence. The formalism is strongly normalizing and confluent, so the order of rule application doesn't affect the final result—a crucial property that ensures our semantic theory remains consistent.
