...
```haskell
-- | Computes functions on states (from Lambda.Delta)
states :: DeltaRule
states = \case
...
```haskell
-- | Note: In the actual Lambda.Delta implementation, the arithmetic rule
-- | handles Add, Mult, and Neg operations, not comparisons.
-- | GE comparisons stay symbolic and are handled by the probabilities rule.
```
...
**Input PDS:**
```haskell
-- Example showing PDS processing of "jo is tall"
-- Functions from Lambda modules:
-- ty :: Sig -> Term -> Typed
-- assert :: Term -> Term  
