...
The entry for *likely* uses the `Pr` operator to compute the probability of a proposition, comparing it to a degree threshold. The `Pr` operator is defined in the signature ([`Lambda.Convenience`](https://juliangrove.github.io/pds/Lambda-Convenience.html)) and reduced by the `probabilities` delta rule in [`Lambda.Delta`](https://juliangrove.github.io/pds/Lambda-Delta.html).
...
This kernel captures that the likelihood that Jo is tall depends on where Jo's height `v` falls relative to the threshold (here normalized to 0). The `normal_cdf` implements vagueness—there's no sharp cutoff but a gradual transition.
...
Our vagueness data looks similar to the norming data but now tracks adjectives separately:
