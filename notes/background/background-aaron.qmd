...
While this methodology has yielded profound insights, it faces serious challenges when we try to scale up our empirical coverage. 
It's important to be clear: the problem is *not* with introspective judgments per se—even formal experiments rely on participants' introspections about acceptability and inference. 
The challenges are a mix of practical and theoretical:
...
Instead of relying on judgments from a few speakers about a handful of examples, experimental semantics collects judgments from large numbers of participants under controlled conditions. This addresses the scale problem directly.

The basic paradigm largely tracks the kinds of questions semanticists ask themselves and their informants: []{@exm-acceptability} []{@exm-truth-conditions} []{@exm-inference-question}
...
By collecting such judgments for many morphems within some class of interest, many contexts in which those morphems occur, and many types of inferences that they might give rise to, one can build detailed profiles of those morphemes' inferential properties for downstream analysis. 
One instance of this approach can be found in the MegaAttitude datasets [@white_computational_2016; @white_role_2018; @white_lexicosyntactic_2018; @white_frequency_2020; @an_lexical_2020; @moon_source_2020; @kane_intensional_2022], which focus on English cause-embedding verbs;
but this methodology is broadly applicable in principle.
...
One challenge that the experimental turn has revealed is that gradience is pervasive in semantic judgments. 
For at least some phenomena, this gradience is unsuprising: 
after all, philosophers and linguists have known about vagueness since antiquity. 
What experimental work reveals is not that gradience exists, but rather its prevalence across phenomena.

Taking examples from the MegaAttitude datasets, @white_role_2018 observe gradient measures of predicates' degrees of factivity, with no clear dividing lines among predicate classes (see also @xue_correlation_2011, @smith_projection_2011, @djarv_prosodic_2017). In neg-raising, @an_lexical_2020 find similar patterns in their MegaNegRaising dataset. And among belief and desire inferences, @kane_intensional_2022 observe gradient patterns in their MegaIntensionality dataset.

The real question is: **What must linguistic knowledge be like to produce the observed patterns of gradience?**

When *know* triggers inference ratings of 0.8 on average (on a 0-1 scale) while *think* triggers ratings of 0.3, what should we conclude? Are these gradient semantic representations or categorical knowledge plus performance factors?
...
When a participant moves a slider to indicate their confidence (or assessment of the likelihood) that an inference follows, what exactly are they telling us? 
Let's unpack in detail what we briefly described earlier as the *inference judgment task*.
The response we observe reflects not just semantic knowledge but a complex cascade of cognitive processes:
